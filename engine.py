...
#     print('anonymizer_img')
    ...
    text_corpus, coordinates, _ = get_text_corpus(img)
...
#         show_result_process(img_array_list, img_array_list_anon)
        ...
        path_to_result = img_to_zip(img_array_list_anon, name_file)
    elif (type_file == '.jpg') or (type_file == '.jpeg'):
        img_array_list = []
...
def img_to_zip(img_array_list_anon, name_file):
    ...
    for num, img in enumerate(img_array_list_anon):
        Image.fromarray(img).save(f'{current_directory}/saving_images/{name_file}/page_' + str(num) + '.jpg', 'JPEG')
